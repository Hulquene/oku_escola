...
-- ========================================================
-- 5. MARCAR EXAMES COMO REALIZADOS (tbl_exam_schedules)
-- ========================================================

-- Marcar Provas do Professor (NPP) como Realizadas
UPDATE `tbl_exam_schedules` 
SET `status` = 'Realizado', 
    `completed_at` = NOW() 
WHERE `id` BETWEEN 1 AND 18;

-- Marcar Provas Trimestrais (NPT) como Realizadas
UPDATE `tbl_exam_schedules` 
SET `status` = 'Realizado', 
    `completed_at` = NOW() 
WHERE `id` BETWEEN 19 AND 39;

-- ========================================================
-- 6. MARCAR PERÍODOS DE EXAME COMO CONCLUÍDOS (tbl_exam_periods)
-- ========================================================

-- Marcar 1ª Época - 1º Trimestre (period_id = 1) como Concluído
UPDATE `tbl_exam_periods` 
SET `status` = 'Concluído' 
WHERE `id` = 1;

-- Marcar 2ª Época - 1º Trimestre (period_id = 2) como Concluído
UPDATE `tbl_exam_periods` 
SET `status` = 'Concluído' 
WHERE `id` = 2;

-- Marcar 1ª Época - 2º Trimestre (period_id = 3) como Concluído
UPDATE `tbl_exam_periods` 
SET `status` = 'Concluído' 
WHERE `id` = 3;

-- ========================================================
-- 7. CONSULTAS PARA VERIFICAR OS DADOS INSERIDOS
-- ========================================================

-- Verificar exames realizados
-- SELECT id, exam_period_id, status, completed_at 
-- FROM tbl_exam_schedules 
-- WHERE status = 'Realizado'
-- ORDER BY id;

-- Verificar períodos concluídos
-- SELECT id, period_name, status 
-- FROM tbl_exam_periods 
-- WHERE status = 'Concluído';

-- Verificar total de notas lançadas por tipo
