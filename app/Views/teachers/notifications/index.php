<?php
// app/Views/teachers/notifications/index.php
?>
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if ($unreadCount > 0): ?>
            <a href="<?= site_url('teachers/notifications/markAllRead') ?>" class="btn btn-outline-primary">
                ...
                <?php foreach ($notifications as $notification): ?>
                    <div class="list-group-item list-group-item-action <?= !$notification->is_read ? 'bg-light' : '' ?>">
                        <div class="d-flex align-items-start">
                            <div class="me-3">
                                <?php 
                                $icon = $notification->icon ?? get_notification_icon($notification->type ?? 'default');
                                $color = $notification->color ?? get_notification_color($notification->type ?? 'default');
                                ?>
                            ...
                            <div class="ms-3">
                                <?php if (!empty($notification->link)): ?>
                                    <a href="<?= site_url('teachers/notifications/read/' . $notification->id) ?>" 
                                       class="btn btn-sm btn-outline-primary">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                <?php endif; ?>
                                <a href="<?= site_url('teachers/notifications/delete/' . $notification->id) ?>" 
